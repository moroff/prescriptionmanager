...
public class TherapyPrescriptionController {
	...
	public String processNewForm(@Valid TherapyPrescription therapy, BindingResult result, Patient patient, ModelMap model) {
        if (result.hasErrors()) {
        	...
        	for (int i = 0; i < therapy.getCount(); i++) {
				nextTherapyDate = calcNextDate(therapy.getPeriodicity(), nextTherapyDate);
			}
        ...
        if (result.hasErrors()) {
        	...
        	if ( stored != null ) {
        		...
        		for (int i = 0; i < stored.getCount(); i++) {
					...
					therapyDate = calcNextDate(therapy.getPeriodicity(), therapyDate);
				}
        		this.prescriptionRepository.save(stored);
        	}
    ...
    public String processAppointmentMoveToEnd(@PathVariable("prescriptionId")int prescriptionId, @PathVariable("appointmentId")int appointmentId, //
    	...
    	appointment.setDate(calcNextDate(prescription.getPeriodicity(), maxDate));
    	prescription.setNextPrescription(calcNextDate(prescription.getPeriodicity(), appointment.getDate()));
    ...
    LocalDate calcNextDate(Periodicity periodicity, LocalDate startDate) {
    	if ( periodicity == null ) {
